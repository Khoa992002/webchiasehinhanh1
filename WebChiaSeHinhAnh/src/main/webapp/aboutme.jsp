			...
			<form action=UpLoadControl method="post"
				...
				<div class="col-12">
					<input name="imgUpload" type="file" class="form-control"
						placeholder="Link hình ảnh"  required accept="image/*">
				</div>
				...
				<div class="col-12">
					<select name="loaihinh" class="form-select"
						...
						<option value="" selected disabled>Loại hình ảnh</option>
			...
			<form action="EditImgControl" method="post" class="row g-3 m-auto"
				...
				<div class="col-12">
					<select name="loaihinh" class="form-select"
						...
						<option value="PNG" ${detail.loaiHinhAnh == 'PNG' ? 'selected' : ''}>PNG</option>
						<option value="JPG" ${detail.loaiHinhAnh == 'JPG' ? 'selected' : ''}>JPG</option>
						<option value="GIF" ${detail.loaiHinhAnh == 'GIF' ? 'selected' : ''}>GIF</option>
					</select>
				</div>
				<div class="col-12">
				...
				<div class="col-12">
					<select name="cate_id" class="form-select"
						aria-label="Default select example">
						<c:forEach items="${listC}" var="C">
							<option value="${C.idCategory}" ${detail.idCategory == 'C.idCategory' ? 'selected' : ''}>${C.cname}</option>
						</c:forEach>
					</select>
				</div>
				<div class="col-12">
					<select name="isPublic" class="form-select"
						aria-label="Default select example" required>
						<option value="1" ${detail.isPublic == '1' ? 'selected' : ''}>Công khai</option>
						<option value="0" ${detail.isPublic == '0' ? 'selected' : ''}>Chỉ mình tôi</option>
					</select>
				</div>
				<div class="col-12"
					style="display: flex; justify-content: flex-end;">
